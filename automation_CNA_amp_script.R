...
for (i in 1:length(my_list)) {
  gene<-my_list[i]
  print(gene)
  data_cna_mod$amplification_status<- ifelse(data_cna_mod$Hugo_Symbol==gene, "", "No Amplification")
  cna_gene<- filter(data_cna_mod, Hugo_Symbol %in% gene)
  cna_gene_uni<- cna_gene %>% distinct(Tumor_Sample_Barcode,.keep_all = TRUE)
  cna_gene_uni$amplification_status<- ifelse(cna_gene_uni$CNA_Type==2, "Amplification", "No Amplification")
  ...
  os_final$amplification_status[is.na(os_final$amplification_status)]<- "No Amplification"
  pfs_final$amplification_status[is.na(pfs_final$amplification_status)]<- "No Amplification"
  fit_os<- survfit(Surv(OS_MONTHS, OS_STATUS) ~ amplification_status, data=os_final)
  fit_pfs<- survfit(Surv(PFS_MONTHS, PFS_STATUS) ~ amplification_status, data=pfs_final)
  png(file= paste0("OS_AMP_", gene, ".png"))
  print(ggsurvplot(
    fit_os, 
    pval = TRUE, 
  ...
  dev.off()
  png(file= paste0("PFS_AMP_", gene, ".png"))
  print(ggsurvplot(
    fit_pfs, 
    pval = TRUE, 
    risk.table = TRUE, risk.table.col = "strata", conf.int = TRUE)
