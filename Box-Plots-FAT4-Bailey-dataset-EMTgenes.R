...
for (i in 1:length(box_plot_list)) {
  ...
  print(emt_gene)
  box_plot_table_gene<- box_plot_table[box_plot_table$Hugo_Symbol %in% emt_gene,]
  ...
  if(p<0.05){
    png(paste0("emt_PC_FAT4_BoxPlot_Bailey", emt_gene ,".png"))
  ...
  write.table(y, file = "genes_emt_p-value_geneexpr_wt_bailey.csv", append = T, col.names = F, row.names = F, sep = ",")
  z<- rbind.data.frame(p_val, cbind.data.frame(emt_gene, p_t))
  write.table(z, file = "genes_emt_p-value_geneexpr_tt_bailey.csv", append = T, col.names = F, row.names = F, sep = ",")
}
