#automate the script
data_patient<- read.delim("D:\\paad_tcga_pan_can_atlas_2018\\data_clinical_patient.txt", as.is = TRUE)
data_mutation<- read.delim("D:\\paad_tcga_pan_can_atlas_2018\\data_mutations.txt", as.is = TRUE)
library(dplyr)
library(survival)
library(survminer)
library(tidyr)
...
rna_expr<- read.delim("D:\\paad_tcga_pan_can_atlas_2018\\data_mrna_seq_v2_rsem.txt", header = TRUE, check.names = FALSE)
...
expr_data_pc$Hugo_Sym[expr_data_pc$Hugo_Sym==""] <- NA

write.csv(expr_data_pc, file = "expr_data_pc.csv")
