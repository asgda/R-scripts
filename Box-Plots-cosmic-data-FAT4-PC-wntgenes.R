...
for (i in 1:length(box_plot_list)) {
  ...
  print(wnt_gene)
  box_plot_table_gene<- box_plot_table[box_plot_table$expr_cols %in% wnt_gene,]
  ...
  if(p_t<0.05){
    png(paste0("WNT_PC_FAT4_BoxPlot_Cosmic_p_t_", wnt_gene ,".png"))
  ...
  write.table(y, file = "genes_wnt_p-value_geneexpr_wt_Cosmic.csv", append = T, col.names = F, row.names = F, sep = ",")
  z<- rbind.data.frame(p_val, cbind.data.frame(wnt_gene, p_t))
  write.table(z, file = "genes_wnt_p-value_geneexpr_tt_Cosmic.csv", append = T, col.names = F, row.names = F, sep = ",")
}
