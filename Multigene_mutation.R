...
data_mutation<- read.delim("C:\\Users\\Avik Sengupta\\Documents\\paad_tcga_pan_can_atlas_2018\\data_mutations.txt", as.is = TRUE)
library(dplyr)
library(survival)
library(survminer)
#sort(table(data_mutation$Hugo_Symbol), decreasing = TRUE)[1:25] = get the most recurrent genes
my_list <- list("KRAS && TP53", "TP53 && MUC16", "MUC16 && SMAD4", "SMAD4 && OBSCN", "OBSCN && PLEC", "PLEC && SYNE1", "SYNE1 && RYR1", "RYR1 & RYR3", "RYR3 && FAT4", "FAT4 && COL5A1", "COL5A1 && DST", "DST && TP53")
my_list
for (i in 1:length(my_list)) {
  genes<-my_list[i]
  print(genes)
  mut<- select(data_mutation, Hugo_Symbol, Variant_Classification, Tumor_Sample_Barcode)
  mut$mutation_status<- ifelse(mut$Hugo_Symbol==genes, "M", "WT")
  mut_gene<- filter(mut, Hugo_Symbol %in% genes)
  ...
  png(file= paste0("OS_MUT_", genes, ".png"))
  print(ggsurvplot(
    fit_os, 
    pval = TRUE, 
  ...
  dev.off()
  png(file= paste0("PFS_MUT_", genes, ".png"))
  print(ggsurvplot(
    fit_pfs, 
    pval = TRUE, 
    risk.table = TRUE, risk.table.col = "strata", conf.int = TRUE)
