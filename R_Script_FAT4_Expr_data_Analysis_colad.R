...
rna_expr<- read.delim("D:\\coadread_tcga_pan_can_atlas_2018\\data_mrna_seq_v2_rsem.txt", header = TRUE, check.names = FALSE)
...
expr_data_cc<- cbind(expr_cols, a)
expr_data_cc$Hugo_Sym[expr_data_cc$Hugo_Sym==""] <- NA

write.csv(expr_data_cc, file = "expr_data_cc.csv")
...
expr_data_cc_1<- read.csv("D:\\Expression\\expr_data_cc.csv", header = TRUE, check.names = FALSE)
names(b)<- b[1,]
b<- b[-c(1),]
...
gene_expr<- expr_data_cc_1$Hugo_Sym
gene_expr<- gene_expr[gene_expr != ""]
gene_expr1<- gene_expr[gene_expr %in% gene_vec]
gene_expr_wnt_cc <- expr_data_cc_1[expr_data_cc_1$Hugo_Sym %in% gene_vec, ]
write.csv(gene_expr_wnt_cc, file = "rna_expr_wnt_cc.csv")
#heatmap
#rm(list=ls())
options(scipen=999)


mat <- read.table("rna_expr_wnt_cc.csv", sep=",", header=T, check.names=F)
mat = mat[,-c(1,2,4)]
rownames(mat) <- mat[,1]
mat[,1] <- NULL 
cols <- colnames(mat)
...
for(i in 1:nrow(mat)){
  zscore <- (mat[i,] - mean(as.numeric(mat[i,]), na.rm = T)) / sd (as.numeric(mat[i,]), na.rm = T)
  write.table(zscore, file="zscore_pan_wnt_cc.csv", col.names=F, sep="\t", quote=F, append=T)
}

mat1 <- read.table("D:\\Expression\\zscore_pan_wnt_cc.csv", sep = "\t", header = F)
mat1 <- mat1[,c(2:ncol(mat1))]
colnames(mat1) <- cols
rownames(mat1) <- rownames(mat)
mat1 <- as.matrix(mat1)

df <- data.frame(Condition = c(rep("MUT", 113), rep("WT", 479)))
ha = HeatmapAnnotation(df = df, col = list(Condition = c("MUT" = "#DD292A", "WT" = "#4FBFAD")), na_col = "grey", show_annotation_name = TRUE)

pdf("Wnt_Heat_Map_FAT4_CC.pdf")
